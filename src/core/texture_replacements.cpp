                                                   ...
                                                   const ReplacementEntry& entry, u32 vram_x, u32 vram_y)
{
  ...
  if (texture->GetWidth() != scaled_width || texture->GetHeight() != scaled_height)
  {
    ...
    Common::ResizeImage(&resized_texture, texture, scaled_width, scaled_height);
...
std::string TextureReplacements::GetSourceDirectory() const
{
  ...
  if (code.empty())
    return g_host_interface->GetUserDirectoryRelativePath("textures");
  else
    return g_host_interface->GetUserDirectoryRelativePath("textures" FS_OSPATH_SEPARATOR_STR "%s", code.c_str());
}
