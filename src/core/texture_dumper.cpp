...
bool TryMergingVRAMWrite(const VRAMWrite& vrw)
{
  ...
  if (vrw.rect.GetWidth() > max_mergee_width || vrw.rect.GetHeight() > max_merge_height)
    return false;
...
void AddPaletteToVRAMWrite(VRAMWrite* vrw, const VRAMWrite::PixelValue& pv)
{
  ...
  switch (pv.mode)
  {
    ...
    {
      const u16* base = &s_vram_shadow[pv.palette_y * VRAM_WIDTH + pv.palette_x];
      const bool transparent = pv.transparent;
      palette.reserve(256);
      for (u32 i = 0; i < 256; i++)
        palette.push_back(ConvertVRAMPixelForDumping(base[i], force_alpha));
    }
...
static bool DumpSubTexture(const VRAMWrite& vrw, GPUTextureMode mode, const U32Rectangle& rect)
{
  ...
  if (FileSystem::FileExists(filename.c_str()) && Common::LoadImageFromFile(&existing_image, filename.c_str()))
  {
...
std::string GetDumpDirectory()
{
  ...
  if (code.empty())
  {
    return g_host_interface->GetUserDirectoryRelativePath("dump" FS_OSPATH_SEPARATOR_STR
                                                          "textures" FS_OSPATH_SEPARATOR_STR);
  }
  else
  {
    return g_host_interface->GetUserDirectoryRelativePath(
      "dump" FS_OSPATH_SEPARATOR_STR "textures" FS_OSPATH_SEPARATOR_STR "%s", code.c_str());
  }
}
...
std::string GetVRAMWriteDumpFilename(u32 width, u32 height, const void* pixels)
{
  ...
  std::string filename = g_host_interface->GetUserDirectoryRelativePath(
    "dump" FS_OSPATH_SEPARATOR_STR "textures" FS_OSPATH_SEPARATOR_STR "%s" FS_OSPATH_SEPARATOR_STR "vram-write-%s.png",
    game_id.c_str(), hash.ToString().c_str());
...
std::string GetTextureDumpFilename(const VRAMWrite& vrw, GPUTextureMode mode, const U32Rectangle& rect)
{
  ...
  std::string filename = g_host_interface->GetUserDirectoryRelativePath(
    "dump" FS_OSPATH_SEPARATOR_STR "textures" FS_OSPATH_SEPARATOR_STR "%s" FS_OSPATH_SEPARATOR_STR
    "texture-%s-%u-%u-%u-%u-%u.png",
    game_id.c_str(), vrw.hash.ToString().c_str(), static_cast<unsigned>(mode), rect.left, rect.top, rect.GetWidth(),
    rect.GetHeight());
