...
bool GameList::LoadEntriesFromCache(ByteStream* stream)
{
  ...
  while (stream->GetPosition() != stream->GetSize())
  {
    ...
    if (!ReadU8(stream, &type) || !ReadU8(stream, &region) || !ReadString(stream, &path) ||
        !ReadString(stream, &ge.serial) || !ReadString(stream, &ge.title) || !ReadString(stream, &ge.genre) ||
        !ReadString(stream, &ge.publisher) || !ReadString(stream, &ge.developer) || !ReadU64(stream, &ge.total_size) ||
        !ReadU64(stream, reinterpret_cast<u64*>(&ge.last_modified_time)) || !ReadU64(stream, &ge.release_date) ||
...
{
  bool result = true;
  result &= WriteU8(m_cache_write_stream.get(), static_cast<u8>(entry->type));
  result &= WriteU8(m_cache_write_stream.get(), static_cast<u8>(entry->region));
  result &= WriteString(m_cache_write_stream.get(), entry->path);
  result &= WriteString(m_cache_write_stream.get(), entry->serial);
  result &= WriteString(m_cache_write_stream.get(), entry->title);
  result &= WriteString(m_cache_write_stream.get(), entry->genre);
  result &= WriteString(m_cache_write_stream.get(), entry->publisher);
  result &= WriteString(m_cache_write_stream.get(), entry->developer);
  result &= WriteU64(m_cache_write_stream.get(), entry->total_size);
  result &= WriteU64(m_cache_write_stream.get(), entry->last_modified_time);
  result &= WriteU64(m_cache_write_stream.get(), entry->release_date);
  result &= WriteU32(m_cache_write_stream.get(), entry->supported_controllers);
  result &= WriteU8(m_cache_write_stream.get(), entry->min_players);
  result &= WriteU8(m_cache_write_stream.get(), entry->max_players);
  result &= WriteU8(m_cache_write_stream.get(), entry->min_blocks);
  result &= WriteU8(m_cache_write_stream.get(), entry->max_blocks);
  result &= WriteU8(m_cache_write_stream.get(), static_cast<u8>(entry->compatibility));
  return result;
}
...
bool GameList::OpenCacheForWriting()
{
  ...
  if (m_cache_write_stream)
  {
    ...
    if (ReadU32(m_cache_write_stream.get(), &signature) && signature == GAME_LIST_CACHE_SIGNATURE &&
        ReadU32(m_cache_write_stream.get(), &version) && version == GAME_LIST_CACHE_VERSION &&
